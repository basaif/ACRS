...
{
    public partial class CarModifyForm : Form
    {
        ...
        List<CarFeatureModel> featuresList = new List<CarFeatureModel>();
        List<CarFeatureModel> selectedFeaturesList = new List<CarFeatureModel>();

        public CarModifyForm(CarModel model)
        {
            InitializeComponent();
            car = model;

            categoriesList = data.GetCarCategoryModels();
            List<CarFeatureModel> allFeatures = data.GetCarFeatureModels();

            foreach (CarFeatureModel feature in allFeatures)
            {
                int featureId = feature.Id;
                bool alreadyUsed = false;

                foreach (CarFeatureModel featureModel in car.Features)
                {
                    if (featureId.Equals(featureModel.Id))
                    {
                        alreadyUsed = true;
        ...
        {
            registerationNumberValue.Text = car.RegisterationNumber;
            yearValue.Text = car.Year;
            makeValue.Text = car.Make;
            modelValue.Text = car.Model;
            pricePerDayValue.Text = car.PricePerDay.ToString();
            mileageValue.Text = car.Mileage.ToString();

            typeComboBox.SelectedItem = car.Category;
            WireUpCategoryLists();
        }
        ...
        private void saveCarButton_Click(object sender, EventArgs e)
        {
            if (ValidateCar())
            {
               data.UpdateCarModel(registerationNumberValue.Text, yearValue.Text, makeValue.Text, modelValue.Text,
                    pricePerDayValue.Text, mileageValue.Text, (CarCategoryModel)typeComboBox.SelectedItem, selectedFeaturesList, car);
